% Complete set of simulations related to entanglement+ tomography with square pulses. Syntax:
%
%                entanglement_diag_new(spin_system,parameters)
%
% Parameters:
%
%	   parameters.readout	 - selection of tomography measurement
%
...
%
% Based on Ilya Kuprov's scripts

function entanglement_diag_new(spin_system,parameters)
...
% echo simulation
echo=powder(spin_system,@entanglement_sequence_wclock,parameters,parameters.assumptions);
...
modulation =fourier*echo;

% Time axis for the DEER trace
x_axis=1e6*linspace(0,parameters.readout_gap,parameters.readout_nsteps+1);
% filename = ['ent_data_' char(string(parameters.main_freq)) '_' parameters.readout '_' char(string(parameters.tau*1e9)) 'ns_' char(string(spin_system.rlx.temperature)) 'K.txt'];
filename = ['ent_data_' char(string(parameters.main_freq)) '_' parameters.readout '_' char(string(parameters.tau*1e9)) 'ns.txt'];
fileID = fopen(filename,'a+');
A = [x_axis; real(modulation); imag(modulation)];
fprintf(fileID,'%6s %12d %12s\n', 'Experiment', parameters.main_freq, parameters.readout);
fprintf(fileID,'%6.4f %12.6f %12.6f\n', A);
fprintf(fileID,'\n');
fclose(fileID);
